...
class SLMLBuilder(object):
    ...
    def build(self) -> str:
        ...
        if(judeg_slml_question_legality(self._detokenize(slml_question))):
            return self._detokenize(slml_question)
        else:
            return modfiy_slml_question(self._detokenize(slml_question))


def judeg_slml_question_legality(slml_question):
    ###added by shhuang,判断是否会出现不匹配的地方。
    slml_question=slml_question.replace("</cm>","\t\t</cm>").replace("</tm>","\t\t</tm>").replace("</vm>","\t\t</vm>").replace("<cm","\t\t<cm").replace("<tm","\t\t<tm").replace("<vm","\t\t<vm")
    slml_question_lst=slml_question.split("\t\t")
    start_lst=[]
    for i in range(len(slml_question_lst)):
        if("<tm" in slml_question_lst[i]):
            start_lst.append("tm")
        elif("<cm" in slml_question_lst[i]):
            start_lst.append("cm")
        elif("<vm" in slml_question_lst[i]):
            start_lst.append("vm")
        if("</tm>" in slml_question_lst[i]):
            temp=start_lst.pop()
            if(temp!="tm"):
...
def modfiy_slml_question(slml_question):
    ###added by shhuang，对出现不匹配的地方进行调整。
    slml_question=slml_question.replace("</cm>","\t\t</cm>").replace("</tm>","\t\t</tm>").replace("</vm>","\t\t</vm>").replace("<cm","\t\t<cm").replace("<tm","\t\t<tm").replace("<vm","\t\t<vm")
    slml_question_lst=slml_question.split("\t\t")
    start_lst=[]
    for i in range(len(slml_question_lst)):
        ...
        if("</tm>" in slml_question_lst[i]):
            temp=start_lst.pop()
            if(temp!="tm"):
                slml_question_lst[i]=slml_question_lst[i].replace("</tm>","</"+temp+">")
        elif("</cm>" in slml_question_lst[i]):
            temp=start_lst.pop()
            if(temp!="cm"):
                slml_question_lst[i]=slml_question_lst[i].replace("</cm>","</"+temp+">")
        elif("</vm>" in slml_question_lst[i]):
            temp=start_lst.pop()
            if(temp!="vm"):
                slml_question_lst[i]=slml_question_lst[i].replace("</vm>","</"+temp+">")
    slml_question="\t\t".join(slml_question_lst).replace("\t\t","")
