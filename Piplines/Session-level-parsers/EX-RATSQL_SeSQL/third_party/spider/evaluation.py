...
class Evaluator:
    ...
    def __init__(self, db_dir, kmaps, etype):
        ...
        self.schemas = {}
        all_database_schema={}
        temp_schema=json.load(open(db_dir))
        for content in temp_schema:
            db_id=content["db_id"]
            table_names_lst=content["table_names"]
            column_names_lst=content["column_names"]
            schema_dic={}
            for index in range(1,len(column_names_lst)):
                column_name=column_names_lst[index][1]
                table_name=table_names_lst[column_names_lst[index][0]]
                if(table_name not in schema_dic):
                    schema_dic[table_name]=[column_name]
                else:
                    schema_dic[table_name].append(column_name)
            all_database_schema[db_id]=schema_dic
        for db_name in self.kmaps.keys():
            self.schemas[db_name] = Schema(all_database_schema[db_name])
        #for db_name in self.kmaps.keys():
