    ...
    for unit in pred_conds:
        ...
        for i, key in enumerate(unit):
            ...
            if isinstance(key, dict):
                for j, unit_2 in enumerate(label_conds):
                    for k, key_2 in enumerate(unit_2):
                        if isinstance(key_2, dict):
                            evaluator = Evaluator()
                            idx = evaluator.eval_exact_match(key_2, key)
                            if idx:
                                label_conds[j] = list(label_conds[j])
                                label_conds[j][k] = 1
                                label_conds[j] = tuple(label_conds[j])
            if idx:
                unit = list(unit)
                unit[i] = 1
                unit = tuple(unit)
...
def eval_and_or(pred, label):
    ...
    # pred_ao = set(pred_ao)       # TODO 不知道为什么用不可哈希的字典来做集合
    # label_ao = set(label_ao)     # TODO 不知道为什么用不可哈希的字典来做集合
...
def evaluate(gold, predict, db_dir, etype, kmaps):
    ...
    for p, g in zip(plist, glist):
        ...
        for gg in g:
            ...
            schema = ALL_DB_SCHEMA[db_name]
            schema = Schema(schema)
            try:
                if g_str == 'select max(覆盖城市数) from 打车APP':
                    print()
        ...
        for idx, pg in enumerate(zip(p, g)):
            ...
            if etype in ["all", "match"]:
                # *********************************************
                # temp_sql = "select 国家 from 台风影响的国家 AS T1 JOIN 台风 AS T2 ON T1.台风id = T2.台风id where 名称 = (select 名称 from 台风影响的中国省份 AS T1 JOIN 台风 AS T2 ON T1.台风id = T2.台风id where 省份 = \"广东\")"
                # if temp_sql == g_str:
                #     print("****************************")
                #     print(p_str)
                #     print(g_str)
...
def get_all_schema(tb_path):
    d = {}
    with open(tb_path, "r", encoding="utf-8") as fp:
        file_data = json.load(fp)
        for db_info in file_data:
            d[db_info["db_id"]] = {}
            for tb_name in db_info["table_names"]:
                d[db_info["db_id"]][tb_name.lower()] = []
            for col_name in db_info["column_names"][1:]:
                d[db_info["db_id"]][db_info["table_names"][col_name[0]].lower()].append(col_name[1].lower())
    return d

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--gold', dest='gold', type=str, default="/opt/data/private/shhuang/IGSQL/data/SeSQL/dev_gold.txt")
    parser.add_argument('--pred', dest='pred', type=str, default="/opt/data/private/shhuang/IGSQL/output_temp copy.txt")
    parser.add_argument('--db', dest='db', type=str, default="/opt/data/private/shhuang/IGSQL/data/database")
    parser.add_argument('--table', dest='table', type=str, default="/opt/data/private/shhuang/IGSQL/data/SeSQL/tables.json")
    ...
    ALL_DB_SCHEMA = get_all_schema(table)
