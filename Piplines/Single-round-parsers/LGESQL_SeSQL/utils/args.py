...
def add_argument_base(arg_parser):
    ...
    arg_parser.add_argument('--task', default='LGESQL_SeSQL_BERT', help='task name')
    arg_parser.add_argument('--seed', default=999, type=int, help='Random seed')
    arg_parser.add_argument('--data_dir', default='data', type=str, help='Data dir')
    ...
    arg_parser.add_argument('--grad_accumulate', default=4, type=int, help='accumulate grad and update once every x steps')
    arg_parser.add_argument('--lr', type=float, default=1e-4, help='learning rate') #原本的学习率是1e-4
    ...
    arg_parser.add_argument('--eval_after_epoch', default=100, type=int, help='Start to evaluate after x epoch')
    arg_parser.add_argument('--load_optimizer', action='store_true', default=False, help='Whether to load optimizer state')
    arg_parser.add_argument('--max_epoch', type=int, default=250, help='terminate after maximum epochs') #原本的max epoch是200
...
def add_argument_encoder(arg_parser):
    ...
    arg_parser.add_argument('--plm', type=str, default='bert-base-chinese' , choices=['bert-base-uncased', 'bert-large-uncased', 'bert-large-uncased-whole-word-masking',
        'roberta-base', 'roberta-large', 'grappa_large_jnt', 'electra-base-discriminator', 'electra-large-discriminator','bert-base-chinese','bret_cn'], help='pretrained model name')
    ...
    arg_parser.add_argument('--attn_drop', type=float, default=0.0, help='dropout rate of attention weights')
    ...
    arg_parser.add_argument('--relation_share_heads', action='store_true', default=True)
...
def add_argument_decoder(arg_parser):
    ...
    arg_parser.add_argument('--no_context_feeding', action='store_true', default=True,
