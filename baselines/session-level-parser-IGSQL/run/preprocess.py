...
def read_database_schema(database_schema_filename, schema_tokens, column_names, database_schemas_dict):
  with open(database_schema_filename, encoding="utf-8") as f:
  ...
  def get_schema_tokens(table_schema):
    ...
    column_names_original = table_schema['column_names'] # TODO
    ...
    for i, (table_id, column_name) in enumerate(column_names_original):
      ...
      column_names.append(column_name.lower())  # TODO 为什么要改成lower()呢？？

    # also add table_name.*
    for table_name in table_names_original:
      column_names_surface_form.append('{}.*'.format(table_name.lower())) # TODO 为什么要改成lower()呢？？
  ...
  for sub_sql in format_sql_2.split('\n'):
    if 'select ' in sub_sql:
      ...
      for t_i, t in enumerate(sub_sql_tokens):
        ...
        # elif (t == 'from' or t == 'join') and 't' not in sub_sql_tokens[t_i+1] and 'T' not in sub_sql_tokens[t_i+1] and sub_sql_tokens[t_i+1] not in table_list:
        #   table_list.append(sub_sql_tokens[t_i+1])
        # TODO 如果一个表没有别名，这里完全不会体现出来
      if len(table_list) > 0:
        ...
        # assert 'join' not in sub_sql    # TODO
        if new_sub_sql is not None:
          sub_sub_sql = sub_sql[:-len(new_sub_sql)].strip()
          assert len(sub_sub_sql.split()) == 2  # TODO ?
          ...
          format_sql_3.append(sub_sql)
        # *********************************** TODO
        elif 'join' in sub_sql:
          temp_sub_sql = sub_sql.split()
          for ii, tok in enumerate(temp_sub_sql):
            if ( tok == 'from' or tok == 'join' ) and temp_sub_sql[ii+1] not in used_tables_list:
              used_tables_list.append([temp_sub_sql[ii+1]])
        # ***********************************
        else:
...
def add_table_name(format_sql_3, used_tables, column_names, schema_tokens):
  ...
  if len(used_tables) == 1:
    ...
    for sub_sql in format_sql_3.split('\n'):
      ...
      for ii, token in enumerate(tokens):
        if (token in column_names or token.lower() in column_names) and tokens[ii-1] != '.':
          if (ii+1 < len(tokens) and tokens[ii+1] != '.' and tokens[ii+1] != '(') or ii+1 == len(tokens):
            if '{}.{}'.format(table_name,token) in schema_tokens or '{}.{}'.format(table_name,token).lower() in schema_tokens:
              tokens[ii] = '{} . {}'.format(table_name,token)
...
def read_data_json(split_json, interaction_list, database_schemas, column_names, output_vocab, schema_tokens, remove_from):
  ...
  for interaction_data in split_data:
    ...
    for turn in interaction_data['interaction']:
      ...
      turn_sql = turn_sql.lower() # lower()一下试试 TODO
      ...
      if remove_from:
        try:
          if turn_sql == 'select 省份 . 名称 from 城市 join 省份 on 城市 . 所属省id = 省份 . 词条id group by 省份 . 名称 having avg ( 城市 . 人口（万） ) > ( select avg ( 城市 . 人口（万） ) from 城市 join 省份 on 城市 . 所属省id = 省份 . 词条id )':
            print()
      ...
      if 'SeSQL' in split_json:
        interaction['interaction'].append({'utterance': turn_utterance, 'sql': turn_sql_parse, 'transfer_way': turn["transfer_way"], 'contextual_phenomena': turn["contextual_phenomena"]})   # TODO 这里添加属于我们数据集的信息
      else:
...
def read_SeSQL(SeSQL_dir, database_schemas, column_names, output_vocab, schema_tokens, remove_from):
  interaction_list = {}

  train_json = os.path.join(SeSQL_dir, 'train.json')
  interaction_list = read_data_json(train_json, interaction_list, database_schemas, column_names, output_vocab, schema_tokens, remove_from)

  dev_json = os.path.join(SeSQL_dir, 'dev.json')
  interaction_list = read_data_json(dev_json, interaction_list, database_schemas, column_names, output_vocab, schema_tokens, remove_from)

  test_json = os.path.join(SeSQL_dir, 'test.json')
  interaction_list = read_data_json(test_json, interaction_list, database_schemas, column_names, output_vocab, schema_tokens, remove_from)

  return interaction_list

def read_db_split(data_dir):
  train_database = []
  with open(os.path.join(data_dir,'train_db_ids.txt'), encoding="utf-8") as f:
    for line in f:
      train_database.append(line.strip())

  dev_database = []
  with open(os.path.join(data_dir,'dev_db_ids.txt'), encoding="utf-8") as f:
    for line in f:
      dev_database.append(line.strip())

  if 'SeSQL' in data_dir:
    test_database = []
    with open(os.path.join(data_dir,'test_db_ids.txt'), encoding="utf-8") as f:
      for line in f:
        test_database.append(line.strip())
    return train_database, dev_database, test_database
...
def preprocess(dataset, remove_from=False):
  ...
  if dataset == 'spider':
    ...
    SeSQL_dir = 'data/SeSQL/'
    database_schema_filename = 'data/SeSQL/tables.json'
    output_dir = 'data/SeSQL_data'
    if remove_from:
      output_dir = 'data/SeSQL_data_removefrom'
    train_database, dev_database, test_database = read_db_split(SeSQL_dir)
  elif dataset == 'SMALL_SeSQL':
    SMALL_SeSQL_dir = 'data/SMALL_SeSQL/'
    database_schema_filename = 'data/SMALL_SeSQL/tables.json'
    output_dir = 'data/SMALL_SeSQL_data'
    if remove_from:
      output_dir = 'data/SMALL_SeSQL_data_removefrom'
    train_database, dev_database, test_database = read_db_split(SMALL_SeSQL_dir)
  ...
  if 'SeSQL' not in dataset:
    print('num_database', num_database, len(train_database), len(dev_database))
  else :
    print('num_database', num_database, len(train_database), len(dev_database), len(test_database))
  ...
  with open(output_database_schema_filename, 'w', encoding="utf-8") as outfile:
  ...
  if dataset == 'spider':
    ...
    interaction_list = read_SeSQL(SeSQL_dir, database_schemas, column_names, output_vocab, schema_tokens, remove_from)
  elif dataset == 'SMALL_SeSQL':
    interaction_list = read_SeSQL(SMALL_SeSQL_dir, database_schemas, column_names, output_vocab, schema_tokens, remove_from)

  print('interaction_list length', len(interaction_list))

  if 'SeSQL' in dataset:
    train_interaction = []
    for database_id in train_database:
      # if database_id not in dev_database and database_id not in test_database:
    ...
    test_interaction = []
    for database_id in test_database:
    ...
    write_interaction(test_interaction, 'test', output_dir)
  else:
    train_interaction = []
...
if __name__ == '__main__':
  ...
  parser.add_argument("--dataset", choices=('spider', 'sparc', 'cosql', 'chase', 'SeSQL', 'SMALL_SeSQL'), default='SMALL_SeSQL')
