...
# modify by shhuang
from utils.evaluation_SeSQL import evaluate, build_foreign_key_map_from_json, build_valid_col_units, rebuild_sql_val, rebuild_sql_col, eval_exec_match
from utils.evaluation_SeSQL import Evaluator as Engine
...
class Evaluator():
    ...
    def sql_acc(self, pred_hyps, dataset, output_path, etype, use_checker):
        ...
        ref_questions = ["".join(ex.question) for ex in dataset]
        ref_question_ids = ['qid'+(6-len(str(i)))*"0"+str(i) for i in range(1,len(dataset)+1)]
        dbs = [ex.db for ex in dataset]
        ...
        with tempfile.NamedTemporaryFile('w+t', encoding='utf8', suffix='.sql') as tmp_pred, \
            ...
            result_type = 'exact' if etype == 'match' else 'exec'
            # modify by shhuang
            all_exact_acc = evaluate(tmp_ref.name, tmp_pred.name, self.database_dir, etype, self.kmaps, ref_questions, ref_question_ids)['all'][result_type]
            sys.stdout = old_print
            of.close()
        return float(all_exact_acc)
...
class Checker():
    ...
    def _get_schemas_from_json(self, table_path):
        ...
        for db in database_list:
            ...
            column_names_original = db['column_names'] # modify by shhuang
            # table_names_original = db['table_names_original']
            table_names_original = db['table_names'] # modify by shhuang
