...
def achieve_db_content_result_dict(db_content_file):
    # modify by shhuang
    db_content_result_dict = {}
    db_content_result = json.load(open(db_content_file))
    for i in db_content_result:
        db_id = i['db_id']
        tables = i["tables"]
        tables_dict = {}
        for table_name in tables:
            cells = tables[table_name]['cell']
            headers = tables[table_name]['header']
            headers_dict = {}
            for header_num in range(len(headers)):
                header_name = headers[header_num]
                headers_dict[header_name] = []
                for cell in cells:
                    headers_dict[header_name].append(cell[header_num])
            tables_dict[table_name] = headers_dict
        db_content_result_dict[db_id] = tables_dict
    return db_content_result_dict


class Preprocessor():

    def __init__(self, db_dir='data/db_content.json', db_content=True, pretrain_model_file='pretrained_models/bert-base-chinese'): # modify by shhuang
        super(Preprocessor, self).__init__()
        self.db_dir = db_dir
        self.db_content = db_content
        self.nlp = stanza.Pipeline('zh', processors='tokenize,pos,lemma')
...
#         ann = self.client.annotate('hello world', ['depparse'])
        self.client = None
        self.stopwords = stopwords.words("chinese")
        self.tokenizer = BertTokenizer.from_pretrained(pretrain_model_file, do_lower_case=True)
        self.db_content_result_dict = achieve_db_content_result_dict(self.db_dir) # modify by shhuang

        # tokenization.FullTokenizer(vocab_file=tokenization_vocab_file, do_lower_case=True) # modify by shhuang
    ...
    def preprocess_database(self, db: dict, verbose: bool = False):
        ...
        for tab in db['table_names']:
            ...
            tab = self.tokenizer.tokenize(tab) # modify by shhuang
            table_toks.append(tab)
            # table_names.append(" ".join(tab))
            table_names.append("".join(tab)) # modify by shhuang
        ...
        for _, c in db['column_names']:
            ...
            c = self.tokenizer.tokenize(c) # modify by shhuang
            column_toks.append(c)
            # column_names.append(" ".join(c))
            column_names.append("".join(c)) # modify by shhuang
    ...
    def preprocess_question(self, entry: dict, db: dict, verbose: bool = False):
        ...
        entry['question_toks'] = self.tokenizer.tokenize(entry["question"]) # modify by shhuang
        raw_toks = list(entry['question_toks']) # modify by shhuang
        toks = list(entry['question_toks']) # modify by shhuang
        ...
        entry['pos_tags'] = None # modify by shhuang
        ...
        # start
        # q_dep_mat = np.array([['question-question-nodependency'] * q_num for _ in range(q_num)], dtype=dtype)
        # q_dep_mat = []
        # use_raw_toks = raw_toks
        ...
        # ann = self.client.annotate(' '.join(use_raw_toks), ['depparse'])
...
#             ann = self.client.annotate(' '.join(use_raw_toks), ['depparse'])
        ...
        #     seg_nodes.append(sent.basicDependencies.node)
        #     seg_edges.append(sent.basicDependencies.edge)
        # _len = 0
        # for nodes, edges in zip(seg_nodes, seg_edges):
        #     for edge in edges:
        #         print("a ", edge.source, edge.target)
        #         source = min(edge.source - 1 + _len, len(use_raw_toks) - 1)
        #         target = min(edge.target - 1 + _len, len(use_raw_toks) - 1)
        #         print("b ",source, target)
        #         q_dep_mat.append([source, target]) 
        #     _len += len(nodes)
        # end

        # if q_num <= MAX_RELATIVE_DIST + 1:
        #    dist_vec = ['question-question-dist-none' + str(i) if i != 0 else 'question-question-identity-none'
        #        for i in range(- MAX_RELATIVE_DIST, MAX_RELATIVE_DIST + 1, 1)]
        #    starting = MAX_RELATIVE_DIST
        # else:
            # dist_vec = ['question-question-generic-none'] * (q_num - MAX_RELATIVE_DIST - 1) + \
            #     ['question-question-dist-none' + str(i) if i != 0 else 'question-question-identity-none' \
            #         for i in range(- MAX_RELATIVE_DIST, MAX_RELATIVE_DIST + 1, 1)] + \
            #         ['question-question-generic-none'] * (q_num - MAX_RELATIVE_DIST - 1)
            # starting = q_num - 1
        # tmp_q_mat = [dist_vec[starting - i: starting - i + q_num] for i in range(q_num)]
        # for element in q_dep_mat:
        #     i=element[0]
        #     j=element[1]
        #     assert i < q_num and j < q_num
        #     print("before : ", tmp_q_mat[i][j])
        #     tmp_q_mat[i][j]=tmp_q_mat[i][j].replace("none","dependency")
        #     print("after : ", tmp_q_mat[i][j])
            
        #     print("before1 : ", tmp_q_mat[j][i])
        #     tmp_q_mat[j][i]=tmp_q_mat[j][i].replace("none","nodependency")
        #     print("after1 : ", tmp_q_mat[j][i])

        # q_mat = np.array(tmp_q_mat, dtype=dtype)
        
        entry['relations'] = q_mat.tolist()
        # entry['dependency_relations'] = q_dep_mat.tolist()
    ...
    def extract_subgraph_from_sql(self, sql: dict, used_schema: dict):
        ...
        select_items = sql['select'] # modify by shhuang
        # select clause
        ...
        if len(orderBy) > 0:
            ...
            for val_unit in orderBy:
                val_unit = val_unit[1] # modify by shhuang
                if val_unit[0] == 0:
                    col_unit = val_unit[1]
                    used_schema['column'].add(col_unit[1])
                else:
    ...
    def schema_linking(self, entry: dict, db: dict, verbose: bool = False):
        ...
        # print("table: ", table_toks)
        # print("column: ", column_toks)
        ...
        for i, j in index_pairs:
            ...
            phrase = ''.join(question_toks[i: j]) # modify by shhuang
        ...
        if self.db_content: # modify by shhuang
            if not os.path.exists(self.db_dir):
                raise ValueError('[ERROR]: database file %s not found ...' % (self.db_dir))
            db_content_result = self.db_content_result_dict[db['db_id']]
            for i, (tab_id, col_name) in enumerate(db['column_names_original']):
                ...
                try:
                    cell_values = db_content_result[tab_name][col_name]
                    cell_values = [[str(float(each))] if is_number(each) else self.tokenizer.tokenize(each) for each in cell_values]
