...
data_dir = params.data_dir  # modify by shhuang
Example.configuration(plm=params.plm, method=params.model, 
                      ...
                      db_dir=os.path.join(data_dir,'db_content.json')) # modify by shhuang
if not args.testing:
    train_dataset, dev_dataset, test_dataset = Example.load_dataset('train'), Example.load_dataset('dev'), Example.load_dataset('test') # modify by shhuang
    logger.info("Load dataset and database finished, cost %.4fs ..." % (time.time() - start_time))
    logger.info("Dataset size: train -> %d ; dev -> %d ; test -> %d" % (len(train_dataset), len(dev_dataset), len(test_dataset))) # modify by shhuang
else:
    dev_dataset, test_dataset = Example.load_dataset('dev'), Example.load_dataset('test') # modify by shhuang
    logger.info("Load dataset and database finished, cost %.4fs ..." % (time.time() - start_time))
    logger.info("Dataset size: dev -> %d ; test -> %d" % (len(dev_dataset), len(test_dataset))) # modify by shhuang
...
if not args.testing:
    ...
    start_epoch, nsamples, best_result = 0, len(train_dataset), {'dev_acc': 0.0, 'test_acc': 0.0} # modify by shhuang
    ...
    for i in range(start_epoch, args.max_epoch):
        ...
        if dev_acc > best_result['dev_acc']:
            best_result['dev_acc'], best_result['dev_iter'] = dev_acc, i
            torch.save({
                'epoch': i, 'model': model.state_dict(),
                'optim': optimizer.state_dict(),
                'scheduler': scheduler.state_dict()
        ...
        if test_acc > best_result['test_acc']:
            best_result['test_acc'], best_result['test_iter'] = test_acc, i
    ...
    logger.info('FINAL BEST DEV RESULT: \tEpoch: %d\tDev acc: %.4f' % (best_result['dev_iter'], best_result['dev_acc']))       # modify by shhuang
    logger.info('FINAL BEST TEST RESULT: \tEpoch: %d\tTest acc: %.4f' % (best_result['test_iter'], best_result['test_acc']))    # modify by shhuang
    ...
    # 对test进行预测和评价！
    start_time = time.time()
    test_acc = decode('test', output_path=os.path.join(args.read_model_path, 'test.eval'), acc_type='sql')
    test_acc_checker = decode('test', output_path=os.path.join(args.read_model_path, 'test.eval.checker'), acc_type='sql', use_checker=True)
    logger.info("Evaluation costs %.2fs ; Test dataset exact match/checker is %.4f/%.4f ." % (time.time() - start_time, test_acc, test_acc_checker))
