...
def evaluate(gold, predict, db_dir, etype, kmaps, gold_questions_list=None, gold_question_ids_list=None):
    if gold.endswith('.json'):
        glist = [(g['query'], g['db_id']) for g in json.load(open(gold))]
    else:
        with open(gold) as f:
            glist = [l.strip().split('\t') for l in f.readlines() if len(l.strip()) > 0]

    if predict.endswith('.json'):
        plist = [(p['query'], p['db_id']) for p in json.load(open(predict))]
    else:
    ...
    eval_err_num = 0
    if(gold_questions_list!=None and gold_question_ids_list!=None):
        for p, g, q, i in zip(plist, glist, gold_questions_list, gold_question_ids_list):
            p_str = p[0]
            g_str, db = g
            db_name = db
            db = os.path.join(db_dir, db, db + ".sqlite")
            # schema = Schema(get_schema(db))
            schema = ALL_DB_SCHEMA[db_name] # modify by shhuang
            schema = Schema(schema) # modify by shhuang
            ...
            if etype in ["all", "match"]:
                ...
                if exact_score == 0:
                    print("question_id: {}".format(i))
                    print("{} pred: {}".format(hardness,p_str))
                    print("{} gold: {}".format(hardness,g_str))
                    print("{} gold_question: {}".format(hardness,q))
                    print("")
...
def build_foreign_key_map(entry):
    ...
    cols_orig = entry["column_names"]  # modify by shhuang
    # tables_orig = entry["table_names_original"]
    tables_orig = entry["table_names"] # modify by shhuang
...
def get_db_schema(table):
    return get_all_schema(table)

# modify by shhuang
ALL_DB_SCHEMA = get_all_schema("data/SeSQL/tables.json")
